...
static void mp4fileinfo(mp4ff_t *mp4, char *info, size_t len)
{
    ...
    if (buffer)
    {
        if (faacDecAudioSpecificConfig(buffer, buffer_size, &mp4ASC) >= 0)
        {
            if (mp4ASC.frameLengthFlag == 1) f = 960.0;
            if (mp4ASC.sbr_present_flag == 1) f *= 2;
        }

        if(faacDecInit2(hDecoder, buffer, buffer_size, &sr, &ch) < 0)
        {
    ...
    faacDecDecode(hDecoder, &frameInfo, buffer, buffer_size);
